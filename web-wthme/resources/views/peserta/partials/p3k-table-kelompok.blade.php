...
<div style="background: rgba(255, 255, 255, 0.2); backdrop-filter: blur(15px); border-radius:1.5rem; overflow:hidden; border: 1px solid rgba(255, 255, 255, 0.4); box-shadow: 0 10px 40px rgba(0, 0, 0, 0.05); margin-bottom:2rem;">
    ...
    <table style="width:100%; border-collapse:collapse; font-size:0.9rem;">
        ...
        @php
            ...
            $formId = 'klp-form-' . $b->id;
            $fileId = 'klp-file-' . $b->id;
        @endphp
            ...
            <td style="padding:1.25rem; text-align:center;">
                @if($fotoUrl)
                    <div style="position:relative; display:inline-block;">
                        ...
                        @if(!$isLocked)
                            <form action="{{ route('peserta.p3k.kelompok.hapus-foto', $b->id) }}" method="POST" style="position:absolute; top:-8px; right:-8px;">
                                @csrf @method('DELETE')
            ...
            <td style="padding:1.25rem; text-align:center;">
                ...
                @else
                    <div style="display:flex; flex-direction:column; gap:0.5rem;">
                        ...
                        @endif
                        @if($item['pengumpulan'])
                        <form method="POST" action="{{ route('peserta.p3k.kelompok.reset', $b->id) }}" onsubmit="return confirm('Reset data barang ini?')">
                            @csrf @method('DELETE')
