{{-- Partial: tabel input barang P3K (kelompok / individu) --}}
@if($data->isEmpty())
<div style="background: rgba(255, 255, 255, 0.2); backdrop-filter: blur(15px); border-radius:2rem; padding:2.5rem 2rem; text-align:center; border:2px dashed rgba(0, 47, 69, 0.2); margin-bottom:2rem;">
    <p style="color:#002f45; opacity:0.6; font-weight:600;">Belum ada daftar barang.</p>
</div>
@else
    ...
    <table style="width:100%; border-collapse:collapse; font-size:0.9rem;">
        <thead>
            <tr style="background: rgba(0, 47, 69, 0.05);">
                ...
                <th style="padding:1.25rem; text-align:center; color:#002f45; min-width:140px;">Input Jumlah</th>
        ...
        @php
            ...
            $formId = $prefix . '-form-' . $b->id;
            $fileId = $prefix . '-file-' . $b->id;
        @endphp
        <tr style="background:{{ $bgRow }}; border-bottom:1px solid rgba(0, 47, 69, 0.05);">
            ...
            <td style="padding:1.25rem; text-align:center;">
                <form method="POST" action="{{ route('peserta.p3k.update', $b->id) }}" enctype="multipart/form-data" id="{{ $formId }}">
                    @csrf @method('PATCH')
            ...
            <td style="padding:1.25rem; text-align:center;">
                @if($fotoUrl)
                    <div style="position:relative; display:inline-block;">
                        ...
                        @if(!$isLocked)
                            <form action="{{ route('peserta.p3k.hapus-foto', $b->id) }}" method="POST" style="position:absolute; top:-8px; right:-8px;">
                                @csrf @method('DELETE')
            ...
            <td style="padding:1.25rem; text-align:center;">
                ...
                @else
                    <div style="display:flex; flex-direction:column; gap:0.5rem;">
                        ...
                        @endif
                        @if($item['pengumpulan'])
                        <form method="POST" action="{{ route('peserta.p3k.reset', $b->id) }}" onsubmit="return confirm('Reset data barang ini?')">
                            @csrf @method('DELETE')
