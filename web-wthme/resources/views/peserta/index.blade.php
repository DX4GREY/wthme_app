    ...
    <div style="min-height:calc(100vh - 64px); padding:3rem 1.5rem; background: linear-gradient(135deg, #e0decd 0%, #bdd1d3 100%);">
        <div style="max-width:800px; margin:0 auto;">
            ...
            <div style="margin-bottom:2.5rem; text-align: center;">
                ...
                <div style="display: inline-block; padding: 0.5rem 1.5rem; background: rgba(255,255,255,0.3); backdrop-filter: blur(10px); border-radius: 2rem; border: 1px solid rgba(255,255,255,0.4); margin-top: 10px;">
                    <p style="color:#002f45; font-size:0.95rem; margin:0; font-weight: 500;">
                        Selamat Datang, <span style="font-weight: 700;">{{ auth()->user()->name }}</span> — Kelompok {{ auth()->user()->kelompok }}
                    </p>
                </div>
            </div>
                    ...
                    @foreach ($pengumuman as $info)
                        <div class="slide-item">
                            <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                                <div style="flex: 1;">
                                    <span style="font-size: 0.65rem; background: #002f45; color: white; padding: 4px 12px; border-radius: 50px; font-weight: 800; letter-spacing: 0.5px; display: inline-block; margin-bottom: 10px;">
                                        {{ strtoupper($info->kategori) }}
                                    </span>

                                    <h4 style="margin: 0 0 12px 0; color: #002f45; font-family: 'Playfair Display', serif; font-size: 1.25rem; font-weight: 700;">
                                        {{ $info->judul }}
                                    </h4>

                                    @if ($info->konten)
                                        <p style="color: #002f45; opacity: 0.8; font-size: 0.95rem; line-height: 1.6; margin-bottom: 15px; font-family: 'Inter', sans-serif;">
            ...
            <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap:1.5rem;">
                @php
                    $menus = [
                        [
                            ...
                            'icon' => '🤳',
                            'title' => 'Daftar Wajah',
                            'desc' => $user->face_registered ? 'Wajah sudah terdaftar ✓' : 'Belum daftar wajah untuk absensi',
                        ...
                        [
                            ...
                            'desc' => $tugasBelum > 0 ? $tugasBelum . ' tugas belum dikumpulkan' : 'Semua tugas sudah dikumpulkan ✓',
                ...
                @foreach ($menus as $menu)
                    ...
                    >
                        @if ($menu['badge'])
                            <span style="position:absolute; top:1rem; right:1.25rem; background:{{ $menu['badge']['color'] }}; color:white; font-size:0.7rem; font-weight:700; padding:0.25rem 0.75rem; border-radius:999px;">
                                {{ $menu['badge']['val'] }}
                            </span>
                        @endif

                        <div style="font-size: 2.2rem; background: rgba(255,255,255,0.5); width: 55px; height: 55px; display: flex; align-items: center; justify-content: center; border-radius: 1rem;">
                            {{ $menu['icon'] }}
                        </div>

                        <div>
                            <h3 style="color:#002f45; font-weight:700; font-size:1.1rem; margin:0 0 0.25rem 0;">{{ $menu['title'] }}</h3>
            ...
            {{-- Footer Info --}}
            <div style="margin-top:2.5rem; background: rgba(0, 47, 69, 0.05); border-radius: 1.5rem; padding: 1.25rem 2rem; border: 1px solid rgba(0, 47, 69, 0.1); display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem;">
                <div style="display: flex; gap: 2rem;">
                    <div>
                        <div style="font-size:0.65rem; color:#002f45; opacity:0.5; text-transform:uppercase; letter-spacing:0.1em;">NIM</div>
                        <div style="color:#002f45; font-weight:700;">{{ auth()->user()->nim }}</div>
                    </div>
                    <div>
                        <div style="font-size:0.65rem; color:#002f45; opacity:0.5; text-transform:uppercase; letter-spacing:0.1em;">Kelompok</div>
                        <div style="color:#002f45; font-weight:700;">{{ auth()->user()->kelompok }}</div>
                    </div>
                </div>
                <div style="color: #002f45; opacity: 0.6; font-style: italic; font-size: 0.8rem; font-weight: 700;">ElektroJoss</div>
            </div>
        </div>
    </div>
