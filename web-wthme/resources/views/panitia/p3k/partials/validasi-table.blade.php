...
<div style="background: rgba(0, 47, 69, 0.8); backdrop-filter: blur(10px); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 1.5rem; padding: 1.25rem 2rem; margin-bottom: 1rem; display:flex; align-items:center; gap:2rem; flex-wrap:wrap;">
    ...
    <div style="flex:1; min-width:200px;">
        ...
        <div style="color:#bdd1d3; font-size:0.75rem; margin-top:0.4rem;">{{ $barangLengkap }} dari {{ $totalBarang }} barang lengkap</div>
    </div>
</div>
        ...
        <table style="width:100%; border-collapse:collapse; font-size:0.85rem;">
            <thead>
                <tr style="background: rgba(0, 47, 69, 0.05);">
                    <th style="padding:1rem 1.5rem; text-align:left; color:#002f45; font-size:0.7rem; text-transform:uppercase;">Barang</th>
                    <th style="padding:1rem; text-align:center; color:#002f45; font-size:0.7rem; text-transform:uppercase;">Target</th>
                    <th style="padding:1rem; text-align:center; color:#002f45; font-size:0.7rem; text-transform:uppercase;">Terkumpul</th>
                    @if($withTerpakai)
                    <th style="padding:1rem; text-align:center; color:#002f45; font-size:0.7rem; text-transform:uppercase;">Terpakai</th>
                    <th style="padding:1rem; text-align:center; color:#002f45; font-size:0.7rem; text-transform:uppercase;">Sisa</th>
                    @endif
                    <th style="padding:1rem; text-align:center; color:#002f45; font-size:0.7rem; text-transform:uppercase;">Bukti</th>
                    <th style="padding:1rem; text-align:center; color:#002f45; font-size:0.7rem; text-transform:uppercase;">Status Validasi</th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr style="border-bottom: 1px solid rgba(255, 255, 255, 0.2); background: rgba(255, 255, 255, {{ $isValidated ? '0.25' : '0.05' }});">
                    ...
                    <td style="padding:1rem; text-align:center;">
                        <div style="font-size:1.1rem; font-weight:800; color:{{ $lengkap ? '#16a34a' : ($terkumpul > 0 ? '#d97706' : '#dc2626') }};">{{ $terkumpul }}</div>
                        <div style="color:#002f45; opacity:0.5; font-size:0.7rem; text-transform:uppercase;">{{ $b->satuan }}</div>
                    </td>

                    @if($withTerpakai)
                    <td style="padding:1rem; text-align:center;">
                        <form action="{{ route('panitia.p3k.terpakai', [$b->id, $kelompok]) }}" method="POST" style="display:flex; gap:0.4rem; justify-content:center; align-items:center;">
                            @csrf
                            <input type="number" name="jumlah_terpakai" value="{{ $item['jumlah_terpakai'] }}" min="0" max="{{ $terkumpul }}"
                            ...
                            <button type="submit" style="background: rgba(0,47,69,0.1); color:#002f45; border:none; padding:0.3rem 0.5rem; border-radius:0.5rem; font-size:0.7rem; cursor:pointer; font-weight:700;">💾</button>
                        </form>
                    </td>
                    <td style="padding:1rem; text-align:center;">
                    ...
                    <td style="padding:1rem; text-align:center;">
                        @if ($item['foto'])
                            <a href="{{ $item['foto'] }}" target="_blank">
                                <img src="{{ $item['foto'] }}" style="width:50px; height:50px; object-fit:cover; border-radius:12px; border:2px solid rgba(255,255,255,0.8);">
                            </a>
                        @else
                    ...
                    <td style="padding:1rem; text-align:center;">
                        @if($item['pengumpulan'])
                            <form action="{{ route('panitia.p3k.validasi', [$b->id, $kelompok]) }}" method="POST">
                                @csrf
                                @if($isValidated)
                                    <button type="submit" style="background: {{ $lengkap ? '#16a34a' : '#d97706' }}; color: white; border: none; padding: 0.5rem 1rem; border-radius: 10px; font-size: 0.75rem; font-weight: 800; cursor: pointer; text-transform: uppercase; width: 110px;">
                                        {{ $lengkap ? '✓ Ter-ACC' : 'ACC Cicilan' }}
                                    </button>
                                @else
                                    <button type="submit" style="background: rgba(0, 47, 69, 0.1); color: #002f45; border: 1px solid rgba(0, 47, 69, 0.2); padding: 0.5rem 1rem; border-radius: 10px; font-size: 0.75rem; font-weight: 700; cursor: pointer; text-transform: uppercase; width: 110px;">
                                        Belum ACC
                                    </button>
                                @endif
                            </form>
