...
@section('content')
    ...
    <div class="page-pad"
        style="min-height:calc(100vh - 64px); padding:2rem 1.25rem; background:linear-gradient(135deg,#e0decd 0%,#bdd1d3 100%);">
        <div style="max-width:1000px; margin:0 auto;">
            ...
            <div class="header-card"
                style="background:rgba(255,255,255,0.3); backdrop-filter:blur(15px); border:1px solid rgba(255,255,255,0.4); border-radius:1.5rem; padding:1.75rem; margin-bottom:2rem; display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:1rem; box-shadow:0 8px 32px rgba(0,47,69,0.1);">
                <div>
                    <h1 class="main-title"
                        style="font-family:'Playfair Display',serif; color:#002f45; font-size:1.85rem; font-weight:800; margin:0;">
            ...
            @if (session('success'))
                <div
                    style="background:rgba(34,197,94,0.2); border:1px solid rgba(34,197,94,0.3); color:#166534; padding:0.85rem 1.25rem; border-radius:1rem; margin-bottom:1.25rem; font-size:0.88rem; font-weight:600;">
                    ✅ {{ session('success') }}</div>
            @endif
            @if ($errors->any())
                <div
                    style="background:rgba(239,68,68,0.15); border:1px solid rgba(239,68,68,0.2); color:#991b1b; padding:0.85rem 1.25rem; border-radius:1rem; margin-bottom:1.25rem; font-size:0.88rem; font-weight:600;">
                    ⚠️ {{ $errors->first() }}</div>
            @endif
            ...
            @php
                ...
                $totalLengkap = collect($summary)
                    ->filter(fn($s) => $s['lengkap'] == $s['total'] && $s['total'] > 0)
                    ->count();
            ...
            <div class="stats-grid"
                ...
                @foreach ([['Jenis Barang', $totalBarang], ['Kelompok', $totalKelompok], ['Sudah Lengkap', $totalLengkap], ['Obat Belum Terima', $totalObatBelum]] as [$label, $val])
                    <div
                        style="background:rgba(0,47,69,0.8); backdrop-filter:blur(10px); border:1px solid rgba(255,255,255,0.1); border-radius:1.25rem; padding:1.25rem 1rem; text-align:center;">
                        <div
                            style="color:#bdd1d3; font-size:0.68rem; text-transform:uppercase; letter-spacing:0.08em; margin-bottom:0.4rem; font-weight:600;">
                            {{ $label }}</div>
                        <div style="color:#d2c296; font-size:2rem; font-weight:800; line-height:1;">{{ $val }}
                        </div>
                    </div>
                @endforeach
            </div>
            ...
            @if ($kelompoks->isEmpty())
                <div
                    style="background:rgba(255,255,255,0.2); backdrop-filter:blur(10px); border-radius:1.5rem; padding:3rem; text-align:center; border:2px dashed rgba(0,47,69,0.2); margin-bottom:2rem;">
            ...
            @else
                <div class="kelompok-grid"
                    style="display:grid; grid-template-columns:repeat(auto-fill,minmax(200px,1fr)); gap:1.25rem; margin-bottom:2.5rem;">
                    @foreach ($kelompoks as $k)
                        ...
                        <a href="{{ route('panitia.p3k.kelompok', $k) }}"
                            ...
                            <div
                                ...
                                @if ($s['obat_belum'] > 0)
                                    <span
                                        style="background:rgba(245,158,11,0.2); color:#92400e; border:1px solid rgba(245,158,11,0.3); font-size:0.6rem; font-weight:800; padding:0.2rem 0.5rem; border-radius:6px; text-transform:uppercase;">{{ $s['obat_belum'] }}
                                        Obat</span>
                                @elseif($allDone)
                                    <span
                                        style="background:rgba(34,197,94,0.2); color:#166534; border:1px solid rgba(34,197,94,0.3); font-size:0.6rem; font-weight:800; padding:0.2rem 0.5rem; border-radius:6px; text-transform:uppercase;">Lengkap</span>
                                @endif
                            </div>
                            <div style="color:#002f45; font-weight:800; font-size:1.05rem; margin-bottom:0.25rem;">Kelompok
                                {{ $k }}</div>
                            <div style="color:#002f45; opacity:0.55; font-size:0.78rem; margin-bottom:0.85rem;">
                                {{ $s['lengkap'] }}/{{ $s['total'] }} barang &middot; {{ $s['obat_total'] }} obat</div>
                            <div style="background:rgba(0,47,69,0.07); border-radius:999px; height:7px; overflow:hidden;">
                                <div
                                    style="background:{{ $allDone ? '#16a34a' : ($pct > 50 ? '#d97706' : '#dc2626') }}; height:100%; border-radius:999px; width:{{ $pct }}%;">
                                </div>
                            </div>
                            <div
                                style="color:#002f45; opacity:0.4; font-size:0.7rem; margin-top:0.35rem; text-align:right;">
                                {{ $pct }}%</div>
                        </a>
                    @endforeach
                </div>
            ...
            @php
                $divisiDiizinkan = ['P3K', 'KONSUMSI', 'LOGISTIK'];
                $canEditStok =
                    auth()->user()->role === 'admin' ||
                    in_array(strtoupper(auth()->user()->divisi ?? ''), $divisiDiizinkan);
            @endphp
            @if (!empty($stokIndividuByMenu) && collect($stokIndividuByMenu)->flatten(1)->isNotEmpty())
                ...
                <h3
                    style="font-family:'Playfair Display',serif; color:#002f45; font-size:1.25rem; font-weight:800; margin:0 0 0.4rem; padding-left:0.25rem;">
                    📊 Stok & Pemakaian Barang Individu</h3>
                <p style="color:#002f45; opacity:0.4; font-size:0.75rem; margin:0 0 1.25rem 0.25rem;">Total dari semua
                    kelompok · Klik tombol — langsung tercatat tanpa reload.</p>

                @foreach ($stokIndividuByMenu as $menu => $barangsStok)
                    @if ($barangsStok->isNotEmpty())
                        <div style="margin-bottom:1.5rem;">
                            <div
                                style="font-size:0.78rem; font-weight:700; color:#002f45; opacity:0.6; margin-bottom:0.5rem; padding-left:0.2rem;">
                                {{ $menuConfig[$menu]['icon'] }} {{ $menuConfig[$menu]['label'] }}
                            </div>
                            <div class="stok-cards"
                                style="display:grid; grid-template-columns:repeat(auto-fill,minmax(230px,1fr)); gap:1rem;">
                                @foreach ($barangsStok as $s)
                                    @php
                                        $b = $s['barang'];
                                        $sisa = $s['total_sisa'];
                                        $terk = $s['total_terkumpul'];
                                        $terp = $s['total_terpakai'];
                                        $pct = $terk > 0 ? round(($sisa / $terk) * 100) : 0;
                                        $cSisa = $sisa <= 0 ? '#dc2626' : ($sisa <= $terk * 0.25 ? '#d97706' : '#16a34a');
                                    @endphp
                                    <div style="background:rgba(255,255,255,0.3); backdrop-filter:blur(12px); border:1px solid rgba(255,255,255,0.45); border-radius:1.35rem; overflow:hidden;"
                                        ...
                                        {{-- Card header --}}
                                        <div
                                            style="padding:0.85rem 1.1rem 0.6rem; background:rgba(0,47,69,0.04); border-bottom:1px solid rgba(0,47,69,0.06);">
                                            <div style="color:#002f45; font-weight:800; font-size:0.9rem;">
                                                {{ $b->nama_barang }}</div>
                                            <div
                                                style="color:#002f45; opacity:0.35; font-size:0.62rem; text-transform:uppercase; letter-spacing:0.04em;">
                                                {{ $b->satuan }}</div>
                                        </div>

                                        {{-- 3 angka --}}
                                        <div
                                            style="display:grid; grid-template-columns:1fr 1fr 1fr; padding:0.85rem 1.1rem 0.65rem; gap:0.5rem;">
                                            <div style="text-align:center;">
                                                <div
                                                    style="color:#002f45; opacity:0.4; font-size:0.55rem; text-transform:uppercase; letter-spacing:0.04em; margin-bottom:0.2rem;">
                                                    Terkumpul</div>
                                                <div class="val-terkumpul"
                                                    style="color:#002f45; font-weight:800; font-size:1.5rem; line-height:1;">
                                                    {{ $terk }}</div>
                                            </div>
                                            <div
                                                style="text-align:center; border-left:1px solid rgba(0,47,69,0.07); border-right:1px solid rgba(0,47,69,0.07);">
                                                <div
                                                    style="color:#92400e; opacity:0.6; font-size:0.55rem; text-transform:uppercase; letter-spacing:0.04em; margin-bottom:0.2rem;">
                                                    Terpakai</div>
                                                <div class="val-terpakai"
                                                    style="color:#d97706; font-weight:800; font-size:1.5rem; line-height:1;">
                                                    {{ $terp }}</div>
                                            </div>
                                            <div style="text-align:center;">
                                                <div
                                                    style="color:{{ $cSisa }}; opacity:0.6; font-size:0.55rem; text-transform:uppercase; letter-spacing:0.04em; margin-bottom:0.2rem;">
                                                    Sisa</div>
                                                <div class="val-sisa"
                                                    style="color:{{ $cSisa }}; font-weight:800; font-size:1.5rem; line-height:1;">
                                                    {{ $sisa }}</div>
                                            </div>
                                        </div>
                                        ...
                                        <div style="padding:0 1.1rem 0.7rem;">
                                            <div
                                                style="background:rgba(0,47,69,0.07); border-radius:999px; height:5px; overflow:hidden;">
                                                <div class="val-bar"
                                                    style="background:{{ $cSisa }}; height:100%; border-radius:999px; width:{{ $pct }}%; transition:width 0.3s;">
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        @if ($canEditStok)
                                            <div
                                                style="padding:0.6rem 1.1rem 0.9rem; border-top:1px solid rgba(0,47,69,0.06); display:flex; align-items:center; gap:0.35rem; flex-wrap:wrap;">
                                                <button type="button" class="btn-stok" onclick="stokAjax(this)"
                                                    data-url="{{ route('panitia.p3k.stok.global.adjust', $b->id) }}"
                                                    data-payload='{"delta":1}' {{ $sisa <= 0 ? 'disabled' : '' }}
                                                ...
                                                <button type="button" class="btn-stok" onclick="stokAjax(this)"
                                                    data-url="{{ route('panitia.p3k.stok.global.adjust', $b->id) }}"
                                                    data-payload='{"delta":-1}' {{ $terp <= 0 ? 'disabled' : '' }}
                                                ...
                                                <input type="number" class="inp-set" min="0"
                                                    max="{{ $terk }}" value="{{ $terp }}"
    ...
    <script>
        ...
        function updateCard(btn, data) {
            ...
            if (bar) {
                bar.style.background = cSisa;
                bar.style.width = (terk > 0 ? Math.round(sisa / terk * 100) : 0) + '%';
            }
            ...
            card.querySelectorAll('.btn-stok').forEach(b => {
                ...
                if (p?.delta === 1) {
                    b.disabled = sisa <= 0;
                    b.style.opacity = sisa <= 0 ? '0.3' : '1';
                }
                if (p?.delta === -1) {
                    b.disabled = terp <= 0;
                    b.style.opacity = terp <= 0 ? '0.25' : '0.65';
                }
            });
        }
