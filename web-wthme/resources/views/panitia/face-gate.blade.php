...
<div style="min-height:100vh; background:linear-gradient(135deg,#e0decd 0%,#bdd1d3 100%); padding:2rem 1.5rem; display:flex; flex-direction:column; align-items:center; gap:1.5rem;">
    ...
    <div style="display:flex; gap:1.5rem; width:100%; max-width:900px; flex-wrap:wrap; align-items:flex-start;">
        ...
        <div style="flex:1; min-width:280px; background:rgba(255,255,255,0.45); backdrop-filter:blur(12px); border:1px solid rgba(255,255,255,0.6); border-radius:2rem; padding:1.75rem;">
            ...
            <div style="margin-bottom:1rem;">
                ...
                <select id="sessionSelect"
                    ...
                    @foreach($activeSessions as $session)
                        <option value="{{ $session->id }}">{{ $session->nama }} ({{ $session->created_at->format('d M, H:i') }})</option>
                    @endforeach
                </select>
            </div>
            ...
            <div style="position:relative; margin-bottom:1rem;">
                ...
                {{-- Scanning overlay --}}
                <div id="scanOverlay" style="display:none; position:absolute; inset:0; border-radius:1.25rem; border:3px solid rgba(16,185,129,0.7); box-shadow:0 0 20px rgba(16,185,129,0.3);"></div>
                <div id="errorOverlay" style="display:none; position:absolute; inset:0; border-radius:1.25rem; border:3px solid rgba(239,68,68,0.7); box-shadow:0 0 20px rgba(239,68,68,0.3);"></div>
            </div>
        ...
        <div style="flex:1; min-width:280px; background:rgba(255,255,255,0.45); backdrop-filter:blur(12px); border:1px solid rgba(255,255,255,0.6); border-radius:2rem; padding:1.75rem;">
            ...
            <div id="logList" style="display:flex; flex-direction:column; gap:0.6rem; max-height:420px; overflow-y:auto;">
    ...
    {{-- Back --}}
    <a href="{{ route('panitia.index') }}" style="color:#002f45; opacity:0.5; font-size:0.85rem; text-decoration:none; margin-top:0.5rem;">
...
// Interval antara capture (ms) — 2.5 detik cukup untuk real-time gate
const SCAN_INTERVAL = 2500;
// Cooldown setelah berhasil (ms) — beri jeda supaya tidak double absen UI
const SUCCESS_COOLDOWN = 4000;

let scanTimer     = null;
let attendedCount = 0;
let isProcessing  = false;
// Set untuk tracking user_id yang sudah masuk ke log UI (bukan DB — DB sudah handle uniqueness)
    ...
    try {
        ...
        if (data.success) {
            if (data.already) {
                ...
                // Pause sebentar setelah sukses baru supaya tidak langsung scan lagi
